...
works but on the other hand it does not harm to understand it and it
will probably easy the understanding of the module usage.

Suppose that we have two modules, Foo and Bar and that Foo provides a
C API that Bar uses generated with Module::CAPIMaker.

When Foo is loaded, the pointers of the functions available through
the C API are published on the global hash C<%Foo::C_API>.

When Bar loads, first, it ensures that Foo is loaded and checks that
the version of the C API required is supported by the version of
Foo. Then, it copies the pointers on the C<%Foo::C_API> hash into C
static storage where they can be efficiently accessed.

Finally calls on Bar to the C functions from Foo are transparently
routed through these pointers with the help of some wrapping macros.

=head2 STRUCTURE OF THE C API

The C API is defined in a file named C<c_api.decl>. This file includes
the prototypes of the functions that will be available through the C
API and configuration settings.

From this file, Module::CAPIMaker generates two sets of files, one set
contains the files that are used by the module providing the C API in
order to support it. The other set is to be used by client modules.
...
A C API as generated by Module::CAPIMaker is comprised of the
following files:

=head3 Client support files

There are two main files to be used by client modules, one is
C<perl_${c_module_name}.c> and contains the definition of a
initialization function. The second file is C<perl_${c_module_name}.h>
containing the prototypes of the functions make available through the
C API and a set of macros to easy their usage.
...
The client files go into the directory C<c_api> where you may also
like to place additional files as for instance a C<typemap> file.
...
Version of the Perl module. This variable should be set from the
Makefile.PL script and is not used internally by the C API support
functions. But it appears on the comments of the generated files.
...
The min_version/max_version numbers are made available through the
%C_API hash and when the client module loads it checks that the
version it requires lays between these two numbers or otherwise it
...
The directory where the client support files are placed

=item module_h_filename

Name of the client support header file.

=item module_c_filename

Name of the client C file providing a function to initilize the client
side of the C API.

=item c_api_h_filename

Name of the support file used on the C API provider side.
...
Foo, setting C<export_prefix=foo_> will make that function available
on the client module as C<foo_bar()>.
...
The text inside this varible is placed at the end of the file
C<${module_c_filename}>.
...
Internally the module uses C<Text::Template> to generate the support
files with a set of default templates. For maximum customizability
...
In order to get the C API interface files regenerated everytime the
...
The module Module::CAPIMaker exports the subroutine C<make_c_api> when
...
Copy the files from the C<c_api> directory into your module directory.
...
=E<gt> '$(O_FILES)'> option into the C<WriteMakefile> arguments:
...
Include the header file provided from the compilation units using the
functions available through the C API.
...
The default set of templates is embedded inside the submodules under
...
Copyright (C) 2012 by Salvador Fandiño, E<lt>sfandino@yahoo.comE<gt>.
